...
bc.train <- full.data[,!remove.factors]
...
set.seed(29) 
inTrain <- createDataPartition(full.train$classe, p = 0.9, list = FALSE) 
bc.train <- full.train[inTrain, -53] 
bc.validation <- full.train[-inTrain, -53] 
classe.train <- full.train[inTrain, 53] 
classe.validation <- full.train[-inTrain, 53] 
```
...
bc.predict <- predict(bc.fit$finalModel, bc.validation)
```

# Model analysis
Simple 2-fold cross validation was carried out using a validation set, with 10% of the original training data. The estimate for the out of sample accuracy is `r signif(bc.predict$results[1, "Accuracy"],3)`. The 95% confidence interval for the accuracy is `r signif((bc.predict$results[1, "Accuracy"] + c(-1, 1) * qnorm(0.975) * bc.predict$results[1, "AccuracySD"]),3)`.
...
kable(confusionMatrix(bc.predict, classe.validation), 
      digits = 3,

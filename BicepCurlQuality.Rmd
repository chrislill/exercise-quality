...
2. Tune for mtry (1:15) and nodesize
...
for(m in 1:15) { 
  rfm.fit <- randomForest(ex.train, classe.train, mtry = m, nodesize = 5)
...
```{r investigate}
library(rpart)
  tree.fit <- train(ex.train, classe.train, method = "rpart")
tree.fit$finalModel
tree.predict <- predict(tree.fit, ex.test1)
confusionMatrix(tree.predict, classe.test1)

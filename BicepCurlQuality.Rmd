...
# Executive summary
A random forest model has been created to predict the quality of a bicep curl based on sensor data. This model was tuned and is expected to have an out of sample error of 0.3%. 

This suspiciously high accuracy indicates overfitting. In this case it is easy to match a specific observation when there are 60 similar observations from the same instance of a bicep curl. An alternative approach would be to select a smaller number of measurements which would be expected to differ for the skeletal movement involved in each type of bicep curl.
...
classe.train <- usable.data[inTrain, 53] 
bc.train <- usable.data[inTrain, -53]
classe.validation <- usable.data[-inTrain, 53] 
bc.validation <- usable.data[-inTrain, -53] 
```
...
g1 <- ggplot(full.data, aes(classe, roll_belt)) + b1 + b2 + b3
g2 <- ggplot(full.data, aes(classe, yaw_belt)) + b1 + b2 + b3
g3 <- ggplot(full.data, aes(classe, pitch_belt)) + b1 + b2 + b3
g4 <- ggplot(full.data, aes(classe, magnet_dumbbell_z)) + b1 + b2 + b3
g5 <- ggplot(full.data, aes(classe, magnet_dumbbell_x)) + b1 + b2 + b3
g6 <- ggplot(full.data, aes(classe, pitch_forearm)) + b1 + b2 + b3
...
The algorith was tuned for mtry. Values of mtry from 1 to 15 were used, and mtry = 9 was found to be the optimum. This code is shown below for information, but not evaluated. It requires that training data is further split to create an additional test set. In future analysis I would use the caret package with `train(method = "rf", tuneGrid = data.frame(mtry = 1:15))`.
...
suppressMessages(bc.predict <- predict(bc.fit, bc.validation))

...
# Executive summary
A random forest model has been created to predict the quality of a bicep curl based on sensor data. This model was tuned and is expected to have an out of sample error of 1.1%. 
...
kable(bc.results$table, 
      digits = 3,
      caption = "Figure 3: Confusion matrix for the final model")
```

  

```{r Importance, cache = TRUE, echo = FALSE, fig.height =8, fig.width = 10}
varImpPlot(bc.fit$finalModel, main = "Figure 4: Importance of each factor in the final model")
```
